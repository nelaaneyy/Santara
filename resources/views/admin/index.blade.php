    ...
    <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div id="content">
            ...
            <div class="container-fluid">
                ...
                <div class="card shadow mb-4">
                    <div class="card-header py-3">
                        <h6 class="m-0 font-weight-bold text-primary">Manajemen Kategori Artikel</h6>
                    </div>
                    <div class="card-body">
                        <table class="table table-bordered">
                            <thead>
                            ...
                            <tbody>
                                @foreach ($kategoris as $kategori)
                                    ...
                                    <div class="modal fade" id="editModal{{ $kategori->id }}" tabindex="-1"
                                        role="dialog" aria-labelledby="editModalLabel{{ $kategori->id }}"
                                        aria-hidden="true">
                ...
                <div class="card shadow mb-4">
                    <div class="card-header py-3">
                        <h6 class="m-0 font-weight-bold text-primary">Support & Feedback</h6>
                    </div>
                    <div class="card-body">
                        <table class="table table-bordered">
                            <thead>
                            ...
                            <tbody>
                                @forelse ($feedbacks as $feedback)
                                    <tr>
                                        ...
                                        <td>{{ ucfirst($feedback->jenis) }}</td>
                                        <td>{{ Str::limit($feedback->isi, 50) }}</td>
                                        <td>
                                            @if ($feedback->is_read)
                                    ...
                                    <div class="modal fade" id="modalFeedback{{ $feedback->id }}" tabindex="-1"
                                        aria-labelledby="modalFeedbackLabel{{ $feedback->id }}" aria-hidden="true">
                                        <div class="modal-dialog">
                                            <form action="{{ route('feedbacks.markAsRead', $feedback->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('PUT')
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title"
                                                            id="modalFeedbackLabel{{ $feedback->id }}">Detail Pesan</h5>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                            aria-label="Close"></button>
                                                    </div>
                                                    <div class="modal-body">
                                                        <strong>Jenis:</strong> {{ ucfirst($feedback->jenis) }}<br>
                                                        <strong>Tanggal:</strong>
                                                        {{ $feedback->created_at->format('Y-m-d H:i') }}<br><br>
                                                        <strong>Isi:</strong>
                                                        <p>{{ $feedback->isi }}</p>
                                                    </div>
                                                    <div class="modal-footer">
                                                        <button type="submit" class="btn btn-success">Tandai Sudah
                                                            Dibaca</button>
                                                    </div>
                                                </div>
                                            </form>
